...
(define (intersection-points circles)
    (if (not (null? circles))
        (append
			(apply append (map (lambda (circle) (intersect (car circles) circle)) (cdr circles)))
...
#|
(sort-points (rate-points '((2 5) (7 8) (10 1) (3 2))))
...
|#
(define (cmp first-list second-list)
	(if (not (null? first-list))
		(if (not (null? second-list))
			(< (car first-list) (car second-list))
			#t
		)
		#t
	)
...
(define (clumped-points points)
	(if (not (null? points))
		(if (even? (length points))
			(prefix-of-list (map (lambda (point) (cadr point)) (sort-points (rate-points points)))
        		(/ (length points) 2))
			(prefix-of-list (map (lambda (point) (cadr point)) (sort-points (rate-points points)))
        		(/ (- (length points) 1) 2))
		)
...
(define (average-point points)
	(if (not (null? points))
		(let (
				(avg-x (/ (apply + (map car points)) (length points)))
          		(avg-y (/ (apply + (map cadr points)) (length points)))
			)
    		(list (distance-product (make-pt avg-x avg-y) points) (make-pt avg-x avg-y))
		)
		'()
	)
)
...
(define (where-am-i distances locations)
	(if (not (null? distances))
		(if (not (null? locations))
			(quicksort (map best-estimate (all-guesses distances locations)) cmp)
			'()
		)
		'()
	)
