...
def findOptimalAlignment(strand1, strand2):
	...
	if (len(strand1) == 0):
		return len(strand2) * ' ', strand2, len(strand2) * -2
	if (len(strand2) == 0):
		return strand1, len(strand1) * ' ', len(strand1) * -2
	...
	if strand1[0] == strand2[0]:
		cache[strand1, strand2] = strand1[0] + bestWith[0], strand2[0] + bestWith[1], bestWith[2] + 1
		return strand1[0] + bestWith[0], strand2[0] + bestWith[1], bestWith[2] + 1 # no benefit from making other recursive calls
	...
	bestWithout = ' ' + bestWithout[0], strand2[0] + bestWithout[1], bestWithout[2] - 2 # penalize for insertion of space

	if bestWithout[2] > best[2]:
		best = (bestWithout[0], bestWithout[1], bestWithout[2])

	# opposite scenario
	bestWithout = findOptimalAlignment(strand1[1:], strand2)
	bestWithout = strand1[0] + bestWithout[0], ' ' + bestWithout[1], bestWithout[2] - 2 # penalize for insertion of space

	if bestWithout[2] > best[2]:
		best = (bestWithout[0], bestWithout[1], bestWithout[2])

	cache[strand1, strand2] = best[0], best[1], best[2]
	return best
...
def main():
	while (True):
		...
		test(alignment[2], plus_str, minus_str, alignment[0], alignment[1])
